...
function makeCard(a, index) {
  ...
  const branchBadge = a.branch
    ? `<span class="alumni-tag blue" style="font-size:10px">${a.branch}</span>` : '';

  div.innerHTML = `
    <div class="card-top-bar"></div>
    <div class="alumni-card-inner">
      <div class="alumni-photo-wrap">
        <div class="alumni-photo" id="card-photo-${a.id}">${photoHtml}</div>
        <div class="upload-photo-btn" onclick="quickUploadPhoto(${a.id})" title="Change Photo">
          <i class="fas fa-camera"></i>
        </div>
      </div>
      <div class="alumni-name">${a.name}</div>
      <div class="alumni-role">${a.role}</div>
      ${a.org ? `<div class="alumni-org"><i class="fas fa-building"></i>${a.org}</div>` : ''}
      ${a.location ? `<div class="alumni-batch"><i class="fas fa-map-marker-alt" style="color:var(--red);margin-right:4px;font-size:10px"></i>${a.location}</div>` : ''}
      <div class="alumni-batch">Batch ${a.batch || '—'}</div>
      <div class="alumni-tags">
        <span class="alumni-tag ${getCategoryClass(a.category)}">${getCategoryLabel(a.category)}</span>
        ${branchBadge}
      </div>
      ${quoteHtml}
      <div class="card-actions">
        <button class="card-btn card-btn-red" onclick="openEditModal(${a.id})"><i class="fas fa-edit"></i> Edit</button>
        <button class="card-btn card-btn-ghost" onclick="openDeleteModal(${a.id})"><i class="fas fa-trash"></i></button>
      </div>
    </div>`;
  return div;
}
